...
class MMAudioSuiteVoCoderLoader:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
...
class MMAudioSuiteFeatureUtilsLoader:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
...
class MMAudioSuitePitchMono:
    ...
    def IS_CHANGED(
        ...
        m = hashlib.sha256()
        m.update(MMAudioSuitePitchMono._audio_digest(audio))
        for v in (n_steps, bins_per_octave, n_fft, win_length, hop_length):
            m.update(int(v).to_bytes(8, "little", signed=True))
        return m.hexdigest()

NODE_CLASS_MAPPINGS = {
    "MMAudioSuiteModelLoader": MMAudioSuiteModelLoader,
    "MMAudioSuiteFeatureUtilsLoader": MMAudioSuiteFeatureUtilsLoader,
    "MMAudioSuiteSampler": MMAudioSuiteSampler,
    "MMAudioSuiteVoCoderLoader": MMAudioSuiteVoCoderLoader,
    "MMAudioSuitePitchMono": MMAudioSuitePitchMono,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "MMAudioSuiteModelLoader": "MMAudioSuite ModelLoader",
    "MMAudioSuiteFeatureUtilsLoader": "MMAudioSuite FeatureUtilsLoader",
    "MMAudioSuiteSampler": "MMAudioSuite Sampler",
    "MMAudioSuiteVoCoderLoader": "MMAudioSuite VoCoderLoader",
    "MMAudioSuitePitchMono": "MMAudioSuite Pitch (Mono)"
    }
